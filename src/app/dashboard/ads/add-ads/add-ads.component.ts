...
export class AddAdsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.params.subscribe(params => {
      ...
      this.adsService.GetId(this.id).
      then( response => { this.ads = response;
        console.log('c')
        setTimeout(() => {
          this.spinner.hide();
        }, this.ads);
        this.mechanicId = this.ads.vendorId;
        this.winchId = this.ads.winchId;
        this.imageSrc = this.ads.image;
        if(this.mechanicId != undefined){
          this.to = "mechanical"
          this.onChange(this.to)
        }else{
          this.to = "winch";
          this.onChange(this.to)
        }
        // let c = new DatePipe('en-us').transform(this.ads.endDate, 'yyyy-MM-dd');
        this.enddate = new Date( this.ads.endDate.replace( /(\d{2})-(\d{2})-(\d{4})/, "$2/$1/$3") );
        console.log(this.enddate)
      });
    })
  }
  GetVendor(){
    this.adsService.GetVendor().
    then( responsedata => { 
      this.vendors = responsedata.data;
    });
  }
  GetWinch(){
    this.adsService.GetWinch().
    then( responsedata => { 
      this.winches = responsedata.data;
    });
  }
  onChange(newValue) {
...
Add(value){
  console.log(this.enddate)
  this.spinner.show()
  let x = new DatePipe('en-Us').transform(this.enddate, 'dd-MM-yyyy');
  console.log(x)
  ...
  this.adsService.Add(value, x).
      then( response => { this.result = response;
       });
      }else{
        this.adsService.Update(value,this.id, x).
           then( response => { this.result = response;
             });
     }
     setTimeout(() => {
