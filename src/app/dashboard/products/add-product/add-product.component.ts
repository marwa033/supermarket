  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(params => {
      ...
      then( response => { this.product = response;
        ...
        this.enname = this.product.name.en;
        this.arname = this.product.name.ar;
...
Add(value){
  ...
  this.productService.Add(value).
      then( response => { this.caategoryresults = response;
       });
      }else{
        this.productService.Update(value,this.id).
           then( response => { this.caategoryresults = response;
             });
     }
     setTimeout(() => {
      this.spinner.hide();
    }, this.caategoryresults);
