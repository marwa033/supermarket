...
export class AddSubComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(params => {
      ...
      this.subService.GetId(this.id).
      then( response => { this.sub = response;
        console.log(this.sub)
        this.enname = this.sub.name.en;
        this.arname = this.sub.name.ar;
        this.categoryId = this.sub.category._id;
        setTimeout(() => {
          this.spinner.hide();
        }, this.sub);
...
Add(value){
  ...
  this.subService.Add(value).
      then( response => { this.result = response;
       });
      }else{
        this.subService.Update(value,this.id).
           then( response => { this.result = response;
             });
     }
     setTimeout(() => {
