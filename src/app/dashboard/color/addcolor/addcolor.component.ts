...
export class AddcolorComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.params.subscribe(params => {
      ...
      this.colorService.GetColorId(this.id).
      then( response => { this.color = response;
        setTimeout(() => {
          this.spinner.hide();
        }, this.color);
        this.enname = this.color.name.en;
        this.arname = this.color.name.ar;
        this.imageSrc = this.color.image;
...
Add(value){
  ...
  this.colorService.Add(value).
      then( response => { this.colorresults = response;
       });
      }else{
        this.colorService.Update(value,this.id).
           then( response => { this.colorresults = response;
             });
     }
     setTimeout(() => {
